...
%15--250 words.
% Classification over imbalanced data, which often lead to poor pediction ability for 
% minority class(es), is a challenging tasks in machine learning. 

% Many algorithms, including \emph{synthetic minority oversampling tenique} (SMOTE) and its derivatives, have been proposed to
% solve this problem by a means of rebanlancing the data distribution via oversampling. However, 
% the effects of these algorithms are still unsatisfactory.  This paper proposes to improve
% the effects of SMOTE by integrating DBSCAN and gaussian distributeion into it.  
% \eat{But these methods are easily affected by noise and 
% overlapping data distributions, and cannot handle clusters form data.} 
% First, minority data points are
% devided into core, borderline and noise points. Then, the total number of oversampling data points is
% assigned for each kind of data points accordingly.  
...
%\vspace{-0.1cm}
% \textbf{Algorithm-level} methods are based on proposing 
...
%  methods\cite{2010Risk} and ensemble methods\cite{Chen_2016}, 
%  and ensemble methods contain boosting\cite{article_boosting} and bagging\cite{2018Multi}.
%  The classic algorithm-based methods include XGBoost\cite{Chen_2016}, LogitBoost\cite{2007LogitBoost}, etc.
%The cost-sensitive methods change the weights or loses to improve the performance of classifier, such as 
%adaboost, which is a lifting algorithm,
...
% \textbf{Undersampling} methods\cite{2004Editorial,2004Minority} remove majority points 
...
% Limit the maximum of $n_{borderline}$ to $multiple\_k\times K $, as shown in Eq.\ref{equation5}.
% \begin{equation}
%   \label{equation5}
...
%   \centering
%   \subfigure[]{
%   \includegraphics[width=.2\textwidth]{myplot2_1.png}
%   %\caption{fig1}
%   }
%   \quad
%   \subfigure[]{
%   \includegraphics[width=.2\textwidth]{myplot2_2.png}
%   }
%   \quad
%   \subfigure[]{
%   \includegraphics[width=.2\textwidth]{myplot2_3.png}
%   }
%   \caption{
%     (a)is initial dataset,(b) and (c) are oversampling results, (c) has limited the number of generation.}
%   \label{fig15}
%   \end{figure}
% In Fig.\ref{fig15}, (a) is initial dataset，
...
% We can find that too many sample points are generated near one sample point in the lower right of Fig.\ref{fig15}(b)
% , thus affecting the overall distribution. 
% Obviously Fig.\ref{fig15}(c) is more consistent with the distribution of the original data.
In order to make the distribution of the generated new sample points 
...
% ODG is used to solve the binary classification problem while the MC-ODG algorithm is used to solve the multi-classification problem. In the ODG algorithm, 
%  we first use the DBSCAN algorithm to cluster minority points.
...
% the data points generated by our methods retains the original data distribution.
% ODG can effectively avoid overlapping and handle noise data points.
